...
import { MessageSquare, Search, Filter, Pin, User, Clock, BookOpen, TrendingUp } from "lucide-react";
...
export default function FormateurDiscussionsPage() {
  ...
  const openDiscussion = (id: string) => router.push(`/formateur/discussions/${id}`);

  return (
    <div className="flex-1 overflow-y-auto bg-gray-50">
        <div className="bg-white border-b border-gray-200 px-6 py-4">
          <h1 className="text-2xl font-bold text-gray-900">Discussions</h1>
          <p className="text-sm text-gray-600 mt-1">Consultez et répondez aux questions des étudiants</p>
        </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-gray-900">{courseOptions.length}</p>
                    </div>
...
value={searchInput}
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-2 flex-wrap">
                              ...
                              {discussion.course_title && (
                                <span className="px-2 py-1 bg-[#03045e] text-white text-xs rounded-md font-medium flex items-center gap-1">
                                  <BookOpen className="w-3 h-3" /> {discussion.course_title}
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-4 text-sm text-gray-500 pt-3 border-t border-yellow-200">
                              <span className="flex items-center gap-1">
                                <User className="w-4 h-4" /> {discussion.user.name}
                              </span>
                              <span className="flex items-center gap-1">
                              ...
                              <span className="flex items-center gap-1">
                                <Clock className="w-4 h-4" /> {formatDate(discussion.created_at)}
                              </span>
                            </div>
                          </div>
                        </div>
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Toutes les discussions</h3>
